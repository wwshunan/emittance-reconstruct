...
def run(current, dx, dxp, dy, dyp):
    ...
    os.system(cmd_str)
    file_renamed = 'testdata\\{0}_{1}_{2}.plt'.format(current[0], current[1], current[2])
    if os.path.exists(file_renamed):
        os.remove(file_renamed)
    os.rename('results\\dtl1.plt', file_renamed)
...
def find_centers(iter_depth, profile_num, nparts, I, bg_noise, basedir):
    #os.chdir('bgworker')
    ...
    #I = 0.9818
    xgrid = 80
    current = np.loadtxt('..\\final-results\\lattice.txt', dtype=int)
    ...
    process_profile(current, iter_depth, bg_noise, partran_dist, i_test, nparts, I, xgrid, basedir)
...
def process_profile(current, iter_depth, bg_noise, partran_dist, i_test, nparts, I, xgrid, basedir):
    x_refer_wires_fname = ['..\\final-results\\x_{0}_{1}_{2}'.format(c[0], c[1], c[2]) for c in current]
    y_refer_wires_fname = ['..\\final-results\\y_{0}_{1}_{2}'.format(c[0], c[1], c[2]) for c in current]
